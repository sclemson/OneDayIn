...
export const editRecommendation = async (req, res) => {
  try {
    ...
    const newUserRecommendations = user.recommendations.filter(
      (rec) =>
        rec.title !== recommendationToEdit.title &&
        rec.location !== recommendationToEdit.location
    );
...
export const DeleteRecommendation = async (req, res) => {
  try {
    ...
    console.log("Recommendation Found");
    const newRecommendations = city.recommendations.filter(
      (rec) => rec._id !== recommendationToDelete._id
    );
    city.recommendations = newRecommendations;
    const newUserRecommendations = user.recommendations.filter(
      (rec) =>
        rec.title !== recommendationToDelete.title &&
        rec.location !== recommendationToDelete.location
    );
    user.recommendations = newUserRecommendations;
    await city.save({ validateModifiedOnly: true });
    await user.save({ validateModifiedOnly: true });
